...
def get_symbol(symbol_code):
    ...
    """
    return session.get("%s/public/symbol/%s" % (URL, symbol_code)).json()

def get_ticker(symbol_code):
    """
    [{
        "ask": "0.050043",
        "bid": "0.050042",
    ...
    """
    return session.get("%s/public/ticker/%s" % (URL, symbol_code)).json()


def get_orderbook(symbol_code):
    """Get orderbook. """
    return session.get("%s/public/orderbook/%s" % (URL, symbol_code)).json()

def get_trading_balance():
    """Get trading balance."""
    return session.get("%s/trading/balance" % URL).json()

def new_order(client_order_id, symbol_code, side, quantity, price=None):
    """Place an order."""
...
def get_order(client_order_id, wait=None):
    ...
    return session.get("%s/order/%s" % (URL, client_order_id), params=data).json()
